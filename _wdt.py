...
def init_graph(point_set, weights):
    ...
    with tf.device('/gpu:'+str(0)):
        ...
        use_weights = weights # tf.minimum(tf.abs(weights), 0.002)
...
if __name__ == '__main__':
    ...
    point_set = np.random.random((n_points, 2))
    
    # randomly initialize weights for each point;
    weights = np.random.uniform(size=n_points) * 0.005 + 0.01
    # 5weights[:] = 1.

    BATCH_SIZE = n_points
                
    session, ops = init_graph(point_set, weights)
